$(function(){
	...
	//var zfburl=window.location.href;
	var zfburl=ut+"/detailswanted.html";
	...
	$.ajax({
		type:"get",
		url:url+"/cli/userneed/getUserOther/1/"+users+"?token="+token+"&mintime="+new Date().getTime(),
		dataType:"json",
		success:function(data){
			gadget_login(data);
			if(data.code==1){
				...
				for(var i=0;i<lenlis;i++){
					...
					html+='</p></td><td><p>';
					var createDates=data.lists[i].createDate.split(" ");
					html+=createDates[0];
					html+='</p></td><td><p>';
					html+=data.lists[i].originPlace;
					html+='</p></td><td><p>';
					html+=data.lists[i].stockPlace;
					html+='</p></td><td><a href="detailswanted.html?name='+encodeURI(data.lists[i].drugName)+'&need='+data.lists[i].userNeedId+'&user='+data.lists[i].userId+'">查看详情</a></td></tr>';
				};
				$("#tbodyo").append(html)
			};
		},
	...
	$.ajax({
		type:"get",
		url:url+"/cli/userneed/getDrugOther/1?token="+token+"&drugname="+names+"&mintime="+new Date().getTime(),
		dataType:"json",
		success:function(data){
			gadget_login(data);
			$("#moreug").attr("href","wantbuy.html?name="+names+"&receipt=&stock=&origin=&quality=");
			if(data.code==1){
				var html='';
				var lenlis=8;
				if(data.lists.length>8){
