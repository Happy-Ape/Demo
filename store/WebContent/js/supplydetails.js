$(function(){
	...
	function winsre(){
		...
		window.location.href="supplydetails.html?name="+supnams+"&receipt="+supreceipts+"&stock="+supstocks+"&origin="+suporigins+"&quality="+supqualitys;
	};
...
//				/$("#sup_are span:contains('"+stockPlace+"')").addClass("sup_color");
	...
	$.ajax({
		type:"get",
		url:url+"/cli/userneed/getall/2/"+open+"/"+str+"/1/1?token="+token+"&drugName="+drugName+"&hasReceipt="+hasReceipt+"&stockPlace="+stockPlace+"&originPlace="+originPlace+"&qualityStandard="+qualityStandard+"&mintime="+new Date().getTime(),
		dataType:"json",
		success:function(data){
			gadget_login(data);
			if(data.code==1){
	...
	function bodytlis(data){
				...
				for(var i=0;i<data.lists.length;i++){
					...
					html+='</p></td><td><p><a  target="_blank" href="buydetails.html?name='+encodeURI(data.lists[i].drugName)+'&need='+data.lists[i].userNeedId+'&user='+data.lists[i].userId+'">查看详情</a></p></td></tr>';
				};
				$("#tbodyt").append(html);
