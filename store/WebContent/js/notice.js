$(function(){
	...
	var selluserid=window.location.search;
		...
		function shoptop(){
			...
			htm+='"  class="heashopa">店铺公告</a><a href="';
		...
		$.ajax({
			type:"get",
			url:url+"/cli/SN/getAllMyNotice/"+selluserid+"/1/"+open+"/"+str+"?mintime="+new Date().getTime(),
			dataType:"json",
			success:function(data){
				gadget_login(data);
				if(data.code==1){
...
//					/console.log(data.message+"message");
	...
	$("#ismytzans").click(function(){
		...
		function notlis(data){
			$(".notice .noticeisdiv").remove();
					var htm ='';
					for(var i=0;i<data.lists.length;i++){
						htm+='<div class="noticeisdiv"><a target="_blank" href="';
						htm+='informationmore.html?s='+data.lists[i].noticeid;
						htm+='">';
						htm+=data.lists[i].title;
						htm+='</a><a class="noticeisa">[';
						htm+=data.lists[i].creattime;
						htm+=']</a></div>';
					};
					$(".notice h5").after(htm);
		};
