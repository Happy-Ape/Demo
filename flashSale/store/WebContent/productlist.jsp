...
<script type="text/javascript">
	...
	$(document)
			.ready(
					function() {
						...
						$
								.ajax({
									url : "http://localhost:8080/store/ProductWeb?m=treelist",
									type : "post",
									data : {
										"data" : strs[1]
									},
									...
									success : function(data) {
										var html = "";
										if (data[0]!= null) {
											html = '<table style="margin-left:180px">';
											for (i = 0; i < data.length; i++) {
												var pro = data[i];
												html += '<tr onclick="showId(this.id)" id="'
														+ pro.nid + '">';

												html += '<td width="150px" style="border-bottom:1px solid red"></td>';
												html += '<td width="300px" style="border-bottom:1px solid red"><img src="http://localhost:8080/'
													+ pro.simg
													+ '" style="width:100px;height:100px />';
												html += '</td><td width="100px"></td>';
												html += '<td width="200px" style="border-bottom:1px solid red">'
														+ pro.sname
														+ '</td>'
														+ '<td width="200px" style="border-bottom:1px solid red">￥ '
														+ pro.nprice
														+ '</td> <td width="350px" style="border-bottom:1px solid red">'
														+ pro.sdescription
														+ '</td></tr>';

											}
										}else {
											html += '<h3 style="margin-left:200px">没找到，请刷新重试或看看其他商品吧</h3>';
										}
										$("#pro").html(html);
									}
								})
	...
	function showId(id) {
		window.location.href = 'http://localhost:8080/store/product.jsp?p='
				+ id;
	}
	function search() {
