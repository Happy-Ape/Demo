$(function(){
	...
	/*$(".mylisp span").click(function(){
		$(".mylisp span").removeClass("mylisspan");
		$(this).addClass("mylisspan");
	});*/
	//alert(state)
	if(state==1){
	...
	$.ajax({
		type:"get",
		url:url+"/cli/userneed/getall/1/"+open+"/"+str+"/"+state+"/2?token="+token+"&mintime="+new Date().getTime(),
		dataType:"json",
	...
	function tbodyslis(data){
		...
		$("#tbodybox").hide();
				...
				for(var i=0;i<data.lists.length;i++){
					...
					html+='"><td><a target="_blank" href="../detailswanted.html?name='+encodeURI(data.lists[i].drugName)+'&need='+data.lists[i].userNeedId+'&user='+data.lists[i].userId+'">';
					...
					html+='</td><td>';
					if(state==-2){
						if(data.lists[i].handleResult==undefined){
							html+="";
						}else{
					...
					html+='</td><td>';
					if(state==0){
						html+='<span class="removebtn">删除</span><a href="buymage.html?type=';
						html+=data.lists[i].userNeedId;
						html+='"><span>修改</span></a>';
					};
					if(state==1){
						html+='<span class="offbtn">下架</span><span class="removebtn">删除</span><a href="buymage.html?type=';
						html+=data.lists[i].userNeedId;
						html+='"><span>修改</span></a>';
					};
					if(state==-1){
						html+='<span class="onbtn">上架</span><span class="removebtn">删除</span><a href="buymage.html?type=';
						html+=data.lists[i].userNeedId;
						html+='"><span>修改</span></a>';
					};
					if(state==-2){
