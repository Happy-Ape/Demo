...
//				?pic=http://7xloj2.com1.z0.glb.clouddn.com/1481009450439
	...
	$("#mybtn").click(function(){
		...
		if(sty!=1){
			$.ajax({
			type:"post",
			url:url+"/cli/userneed/save?token="+token+"&userType=1&drugImages="+drugImages+"&drugName="+drugName+"&drugSpecifications="+drugSpecifications+"&number="+snumber+"&userName="+userName+"&userContact="+userContact+"&originPlace="+originPlace+"&stockPlace="+stockPlace+"&hasReceipt="+hasReceipt+"&qualityStandard="+qualityStandard+"&mintime="+new Date().getTime(),
			dataType:"json",
			success:function(data){
				gadget_login(data);
				if(data.code==1){
			...
			$.ajax({
			type:"post",
			url:url+"/cli/userneed/update/"+unId+"?token="+token+"&userType=1&drugImages="+drugImages+"&drugName="+drugName+"&drugSpecifications="+drugSpecifications+"&number="+snumber+"&userName="+userName+"&userContact="+userContact+"&originPlace="+originPlace+"&stockPlace="+stockPlace+"&hasReceipt="+hasReceipt+"&qualityStandard="+qualityStandard+"&mintime="+new Date().getTime(),
			dataType:"json",
			success:function(data){
				gadget_login(data);
				if(data.code==1){
					//gadget_popupt("修改成功");
					//window.location.href="buymage.html";
					gadget_popupfs("修改成功，是否返回需求列表",0,"buyshop.html","刷新","返回")
				}else{
					gadget_popupt("修改失败，"+data.message);
				};
			},
			error:function(){
				gadget_popupt("修改失败，请刷新重试");
			}
		});
		};
